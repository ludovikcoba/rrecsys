...
## What is rrecsys?
Provides implementations of several popular recommendation systems. They can process standard recommendation datasets (user/item matrix) as input and generate rating predictions and recommendation lists. The package is intended for rapid prototyping of recommendation algorithms and education purposes. Currently on rrecsys are developed the following non-personalized RS algorithms:
...
```
Once the package is installed the usual thing to do would be to load the it:
```{r, eval=FALSE}
library(rrecsys)
```
