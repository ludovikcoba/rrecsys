...
evalChart <- function(res, x = "items", y = "TP", x_label, y_label, y_lim){
  ...
  if (x == "items"){
    if (y == "TP"){
      ...
      if(missing(y_label)){
        plot_PopDist_freq(res@data, res@rec_counts, y_lim)
      }else{
        plot_PopDist_freq(res@topN, res@rec_counts, y_label = y_label, y_lim)
      }
    ...
    if(y == "num_of_ratings"){
      ...
      if(missing(y_label)){
        plot_topN_popularity(res@topN, res@rec_popularity)
      }else{
        plot_topN_popularity(res@topN, res@rec_popularity, y_label = y_label)
      }
